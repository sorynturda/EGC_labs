...
namespace egc {
    ...
    mat4 defineCameraMatrix(Camera mc)
    {
        ...
        egc::mat4 m2 = mat4();// egc::translate(-mc.cameraPosition.x, -mc.cameraPosition.y, -mc.cameraPosition.z);
        m2.matrixData[12] = -mc.cameraPosition.x;
        m2.matrixData[13] = -mc.cameraPosition.y;
        m2.matrixData[14] = -mc.cameraPosition.z;
        cameraMatrix = m1 * m2;
        return cameraMatrix;
    }
    ...
    bool clipPointInHomogeneousCoordinate(const egc::vec4 &vertex)
    {
        ...
        if ((-abs(vertex.w) <= vertex.x && vertex.x <= abs(vertex.w)) &&
            (-abs(vertex.w) <= vertex.y && vertex.y <= abs(vertex.w)) &&
            (-abs(vertex.w) <= vertex.z && vertex.z <= abs(vertex.w)))
            return false;
        return true;
    }
    ...
    egc::vec4 findCenterPointOfTriangle(const std::vector<egc::vec4> &triangle)
    {
        ...
        //triangleCenter.x = (triangle[0].x + triangle[1].x + triangle[2].x) / 3;
        //triangleCenter.y = (triangle[0].y + triangle[1].y + triangle[2].y) / 3;
        //triangleCenter.z = (triangle[0].z + triangle[1].z + triangle[2].z) / 3;
